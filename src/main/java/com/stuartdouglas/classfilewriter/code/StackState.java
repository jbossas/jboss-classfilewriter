...
public class StackState {
    ...
    public StackState pop() {
        ...
        if (type.getType() == StackEntryType.TOP) {
            throw new RuntimeException("Cannot pop wide type");
        }
    ...
    public StackState pop2() {
        if (contents.size() < 2) {
            throw new RuntimeException("cannot pop2, only " + contents.size() + " on stack " + toString());
        }
        StackEntry type = top_1();
        if (type.isWide()) {
            throw new RuntimeException("Cannot pop2 when second type on stack is wide " + toString());
        }
    ...
    public StackState dup() {
        if (contents.isEmpty()) {
            throw new RuntimeException("cannot dup empty stack");
        }
        StackEntry type = top();
        if (type.getType() == StackEntryType.TOP) {
            throw new RuntimeException("Cannot dup wide type");
        }
    ...
    public StackState dup2() {
        if (contents.size() <2) {
            throw new RuntimeException("cannot dup2, stack size is " + contents.size() + " " + toString());
        }
        StackEntry t1 = top();
        StackEntry t2 = top_1();
        if (t2.getType() == StackEntryType.TOP) {
            throw new RuntimeException("Cannot dup2 when second type on stack is wide: " + toString());
        }
